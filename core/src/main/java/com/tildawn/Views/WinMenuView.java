...
public class WinMenuView implements Screen {
    private Stage stage;
    private Table table;
    ...
    public WinMenuView(GameController gameController, Skin skin) {
        GameAssetManager.getGameAssetManager().getWinSound().play();
        table = new Table();
        youWinLabel = new Label("YOU SURVIVED", skin, "title");
        ...
        App.getLoggedInUser().setMaxAliveTime(Math.max(App.getLoggedInUser().getMaxAliveTime(), gameController.getElapsedTime() / 60));        scoreLabel = new Label("score : " + score, skin, "subtitle");
